...
#define VERSION         "[2019-04-27]"
#define AUTHOR          "loveemu, tcdw"
...
static bool nintSpcIgnoreQ = false;
...
static int nintSpcCheckVer (NintSpcSeqStat *seq)
{
    ...
    if (nintSpcAutoQFix && seq->ver.durTableAddr != -1 && seq->ver.velTableAddr != -1) {
		...
		// standard nspc / amk
		const byte smwVel[] = { 0x19, 0x33, 0x4C, 0x66, 0x72, 0x7F, 0x8C, 0x99, 0xA5, 0xB2, 0xBf, 0xCC, 0xD8, 0xE5, 0xF2, 0xFC };
        const byte *dst, *src;
        int *result;
        int tableLen;
        int i, j;
...
static void nintSpcEventNoteInfo (NintSpcSeqStat *seq, SeqEventReport *ev)
{
    ...
    if (hasNextArg) {
        ...
        default:
            ...
            durRateIndex = nintSpcIgnoreQ ? 7 : (arg2 >> 4) & 7;
            velIndex = arg2 & 15;
            ...
            if (!seq->looped) {
                int fixedQVal = (nintSpcMMLDurFix[nintSpcIgnoreQ ? 7 : ((arg2 >> 4) & 7)] << 4) | nintSpcMMLVelFix[arg2 & 15];
...
static void nintSpcSetEventList (NintSpcSeqStat *seq)
{
    ...
    case SPC_VER_OLD:
        ...
        event[vcmdStart+0x13] = (NintSpcEvent) nintSpcEventUnknown2;		//ADSR切换
		...
		event[0xF3] = (NintSpcEvent) nintSpcEventUnknown2;		//音色切换
		event[0xF4] = (NintSpcEvent) nintSpcEventUnknown1;
		event[0xF5] = (NintSpcEvent) nintSpcEventUnknown8;
		event[0xF6] = (NintSpcEvent) nintSpcEventUnknown2;		//DSP写入
		event[0xF7] = (NintSpcEvent) nintSpcEventUnknown3;		//ARAM写入
		event[0xF8] = (NintSpcEvent) nintSpcEventUnknown1;
		event[0xFA] = (NintSpcEvent) nintSpcEventUnknown1;
...
static CmdOptDefs optDef[] = {
	...
	{ "ignoreq", '\0', 0, cmdIgnoreQ, "", "Ignore 'q' curve" }
};
